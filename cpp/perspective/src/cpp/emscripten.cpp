...
namespace binding {
    ...
    get_aggs(const std::vector<std::string>& columns,
        const std::vector<std::vector<std::string>>& sort, t_val j_aggs) {
        return std::map<std::string, std::string>{};
    }

    std::vector<std::tuple<std::string, std::string, t_tscalar>>
    get_filter(const t_schema& schema, t_val j_date_parser, t_val j_filter) {
        return std::vector<std::tuple<std::string, std::string, t_tscalar>>{};
    }

    std::vector<std::vector<std::string>>
    get_sort(const std::vector<std::string>& columns, t_val j_sort, bool is_column_sort) {
    ...
    make_view_config2(
        ...
        t_val j_filter_op = config["filter_op"];
        ...
        if (has_value(j_sort)) {
            sort = get_sort(columns, j_sort, false);
        }

        if (has_value(j_filter)) {
            filter = get_filter(schema, date_parser, j_filter);
        }

        // agg needs columns and sort
        aggregates = get_aggs(columns, sort, j_aggregates);

        // FIXME: intricacies around combiners, etc.

        auto view_config = t_view_config(
            row_pivots, column_pivots, columns, aggregates, filter, sort, column_only);
